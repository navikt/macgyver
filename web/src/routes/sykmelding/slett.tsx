...
function SlettSykmelding(): ReactElement {
    const { data, isSuccess, isPending, error, mutate } = useMutation({
        mutationFn: async (params: { sykmeldingId: string; journalpostId: string | null }) => {
            ...
            return await fetchApi(`/sykmelding/${params.sykmeldingId}/${params.journalpostId ?? 'null'}`, {
                method: 'DELETE',
    ...
    return (
        <BasicPage title="Slett sykmelding" ingress="Sletter en sykmelding" hasAuditLog>
            <SlettSykmeldingForm

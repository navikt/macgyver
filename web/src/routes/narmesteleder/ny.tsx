...
function NyNarmesteleder(): ReactElement {
    const { data, isSuccess, isPending, error, mutate } = useMutation({
        mutationFn: async (params: { sykmeldingId: string; fnr: string; orgnummer: string }) => {
    ...
    return (
        <BasicPage title="NL-request til Altinn" ingress="Sender ny NL-request til Altinn" hasAuditLog>
